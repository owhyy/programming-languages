...
class Piece
  ...
  def initialize (point_array, board)
    ...
    @rotation_index = (0..(@all_rotations.size-1)).to_a.sample # random initial rotation
  ...
  def move (delta_x, delta_y, delta_rotation)
    ...
    moved = true # used to check if it can move
    potential = @all_rotations[(@rotation_index + delta_rotation) % @all_rotations.size] # next rotation
  ...
  # this basically creates all possible rotations of a piece
  ...
  def self.next_piece(board)
    Piece.new(All_Pieces.sample, board) # adds a random element, in a random rotation position to the board
  end

  # class array holding all the pieces and all of their rotations
  All_Pieces = [[[[1, 0], [1, 0], [0, 1], [1, 1]]],  # square (only needs one)
...
class Board
  ...
  def empty_at (point)
    ...
    @grid[point[1]][point[0]] == nil # row(y) is first, column(x) is second
  end
