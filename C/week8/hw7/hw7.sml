...
fun eval_prog (e,env) =
    case e of
     ...
     (case List.find (fn (s2,v) => s=s2) env of
       ...
       | Shift(deltaX, deltaY, e) => let val exp = eval_prog (e, env) in
         case exp of
              NoPoints => NoPoints
            | Point (x, y) => Point (x + deltaX, y + deltaY)
            | Line  (m, b) => Line  (m, (b + deltaY) - (m * deltaX))
            | VerticalLine x => VerticalLine (x + deltaX)
            | LineSegment (x0, y0, x1, y1) => LineSegment (x0 + deltaX,
              y0 + deltaY, x1 + deltaX, y1 + deltaY)
                                     end
...
fun preprocess_prog e =
    case e of
     ...
     val (x0,y0,x1,y1) = seg
     in
     if real_close_point((x0, y0)) (x1, y1)
     then Point(x0, y0) (* point conversion *)
     else if real_close(x0, x1)
     then if (y0 < y1)
          then LineSegment(x0, y0, x1, y1)
          else LineSegment(x0, y1, x1, y0)
     else LineSegment(x1, y1, x0, y0)
     end
       | _ => e
